...
这个 quickstart 使用 `gopact-ext/devagent/selfbootstrap` 和 `gopact-ext/devagent/workspace` 运行一条无凭据 Dev Agent self-bootstrap workflow。示例会创建临时 git 仓库，由 planner 产出 patch proposal，经本地 policy 授权后通过 workspace adapter 应用 approved plan patch，采集 repo-relative worktree diff 与 file snapshot，在临时 workspace 内执行 `go test ./...`，校验 quickstart release requirements，并输出 run export 和 verification evidence 摘要。
