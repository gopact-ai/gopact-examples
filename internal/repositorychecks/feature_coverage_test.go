...
func TestFeatureCoverageMatrixDocumentsExpectedCapabilities(t *testing.T) {
	...
	tests := []struct {
		...
		{
			capability:  "core agent init/run scaffold",
		...
		{
			capability:  "Dev Agent self-bootstrap workflow with workspace patch apply, diff, file snapshot, command, CI gate, run export, failure attribution, and verification report evidence",
