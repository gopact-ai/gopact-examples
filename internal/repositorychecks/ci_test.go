...
func TestExamplesUseCurrentReleasedModules(t *testing.T) {
	...
	for _, requirement := range []string{
		"github.com/gopact-ai/gopact v0.0.35",
	...
	if !strings.Contains(generatedAgent, `gopactVersion = "v0.0.35"`) {
		t.Fatal("quickstart/generated-agent must exercise gopact agent init at current core SDK v0.0.35")
	}
}
