...
> **仅支持 Go 1.27 及以上版本。** 本项目使用泛型方法，因此需要 Go 1.27 或更高版本。

Go 1.27.0 正式工具链尚未出现在公共下载服务，因此 CI 暂时固定为
`1.27.0-rc.2`。gopact-examples 后续只发布稳定语义版本，不再新增项目候选版本。

手动执行的源码端到端流程要求传入经过审查的 gopact 与 gopact-ext
40 位提交 SHA。它会检出这些精确提交，打印三个仓库的 SHA，再用临时
Go 工作区联调。日常 CI 则在关闭 Go 工作区（`GOWORK=off`）的情况下，
使用本模块声明的不可变稳定版本。
