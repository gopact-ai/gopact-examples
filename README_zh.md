...
| `quickstart/agent-cluster` | 本地 A2A cluster、`Mesh.SyncEnv` discovery、policy、retry、cancel、dev-agent replay 和 command evidence。 | 否 |
...
agent-cluster quickstart 使用 `Mesh.SyncEnv` 导入环境变量配置的 agent cards，注册可调用 HTTP agents，并在路由任务前剔除未就绪 endpoint。
